...
module seqDetect_tb;
    ...
    seqDetect dut(.valid(valid), .a(a), .seq(seq), .clk(clk), .reset_n(reset_n));
    ...
    initial begin
        ...
        reset_n = 0;
        a = 0;
        // set a random 3 bit value to seq (from ChatGPT)
        $random(42);
        seq = $random % 8;  // between 0 and 7
        $display("Random sequence to detect: %b", seq);
        #10;    // Wait a bit
